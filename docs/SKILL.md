...
Log pipeline secret leakage detector.
...
```
go install github.com/ppiankov/logspectre/cmd/logspectre@latest
```
...
Scans log pipelines for security findings.
...
- Does not remediate or modify log pipelines — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated log pipelines monitoring — point-in-time security audit only
...
```bash
logspectre scan --output json | jq '.summary'
logspectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
